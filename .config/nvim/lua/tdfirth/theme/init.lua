...
vim.g.colors_name = "tdfirth"

C = require("tdfirth.theme.colors")

local highlights = {}

for _, module in ipairs({ "base", "lsp" }) do
	highlights = vim.tbl_deep_extend("force", highlights, require("tdfirth.theme." .. module))
end
...
for plugin, enabled in pairs({
	...
	if enabled then
		highlights = vim.tbl_deep_extend("force", highlights, require("tdfirth.theme.plugins." .. plugin))
	end
end
